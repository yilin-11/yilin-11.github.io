...
`shelfie.html` is the only page that calls a network API: four `fetch` calls to `https://api.anthropic.com/v1/messages` for receipt parsing, recipe generation, and recipe import from text/URL/screenshot. Notes if you touch them:

- The pantry has **two entrances — receipt scan and typed entry — and both land in the same editable `detected[]` review list**; `commitReview()` is the only writer into `pantry`. Keep that funnel: adding a third source means feeding the same review, not a second path into `pantry`. `addErr()` renders every scan failure with a "type it in instead" button, which is what keeps the AI path from being a dead end — don't reduce it to a plain message. A camera-on-the-counter entrance existed through several iterations and was cut: it could not see inside packaging, read a weight or price anything, so it did the receipt's job badly. Don't reintroduce it without that argument being answered.
...
- **Dark is the default theme**, resolved by an inline `<head>` script into `data-theme` before the first paint (an external file would flash the light theme first). Dark values live on `:root`; light is the single `:root[data-theme="light"]` override. The choice is stored in `localStorage` under `shelfie-theme` — deliberately *not* in the app's `STORE`/account, since it is a property of the device and has to be readable synchronously. Every colour is a token: `--green` is always a surface white sits on, `--green-deep` is always text, and `--green-hover` exists because one token cannot be both.
- **The account is created without an email.** `startGuest()` enters the app on one tap; `signUp()` is reachable only from the keep-it sheet (`openSave()`), which `maybeAskToKeep()` raises once, after a haul lands or a meal is logged, and never again (`nagged`). Don't move the email back to the first screen — the case study's argument is that value is delivered before anything is asked for.
- `--tabh` is the scroll clearance under the tab bar; `--tabbar-h` is what the bar actually occupies. Anything floating above the tabs pins to `--tabbar-h` (see `.revbar`), and `position:sticky` inside `.view` does **not** resolve against the phone bottom the way you would expect — that bug hid the review's commit button behind the tabs. Use an absolutely positioned bar beside the view, as `.rd-bar` and `.revbar` both do.
