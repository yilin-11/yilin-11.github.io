...
/* ---------- 04 · MUTUO ---------- */
{
  ...
  type:'Full-stack repair · failure states · ranking', stack:'Express · Sequelize · Passport',
  ...
  shots:[{src:'mutuo-directory.jpg',cap:'People nearby: a possible swap is labelled outright, and it outranks a shorter walk.'},
         {src:'mutuo-matches.jpg',cap:'Matches, mutual ones first — the state that used to exist but tell nobody.'}],
  chapters:[
...
<b>Members</b> — three pages, in the order you need them
    <b>People nearby</b> · search · Within control · random match · member detail
...
<b>Data</b>
   ...
   <p>The third pass settled the order. Nearby comes first because it is the reason to open the app, and your own profile comes last because you write it once. The random match stopped being a destination and became a button on the nearby page — <b>deciding who to ask is something you do while looking at the list</b>, not on a screen of its own. And anything that depends on <i>who is asking</i> — how far away someone is, whether the two skill sets line up, whether you have matched, whether they matched back — is computed per request rather than stored, because none of it is a property of the member being looked at.</p>`,
  `<p>Then the part that was actually missing: the states nobody had drawn. Each screen had one design — the happy one. These are the rest, and every one of them is now a deliberate choice rather than an accident.</p>
   ...
   <ul>
     <li><b>A possible swap outranks a short walk.</b> Every card now says outright what it is — <i>Straight swap</i>, <i>Teaches what you want</i>, <i>Wants what you teach</i> — and the ordering leads with those, distance deciding within each group. Complementarity is the harder constraint: a neighbour who teaches nothing you want is not a trade at all.</li>
     <li><b>Nearby means nearby.</b> A postal code is resolved to coordinates once, when the profile is saved, so distance is arithmetic at read time rather than fifty lookups against a service that permits about one a second. A <b>Within</b> control bounds the list and everything past it folds into <i>Farther away</i> rather than vanishing — a page called "people nearby" that leads with someone 16,000 km away is arguing with its own title. A member who cannot be placed sorts to the end instead of disappearing.</li>
